...
const mockCreateContainer = mock(() => Promise.resolve({
  id: 'abc123',
  name: 'test-container',
  image: 'nginx:latest',
  status: 'running',
...
const mockGetContainer = mock(() => Promise.resolve({
  id: 'abc123',
  name: 'test-container',
  image: 'nginx:latest',
  status: 'running',
...
const mockListContainers = mock(() => Promise.resolve([
  {
    id: 'abc123',
    name: 'test-container',
    image: 'nginx:latest',
...
describe('Container Routes', () => {
  ...
  test('POST /containers - 컨테이너 생성', async () => {
    const res = await app.request('/containers', {
      ...
      body: JSON.stringify({ name: 'test-container' })
    });
