...
import { createContainerSchema } from '../../lib/schemas/containers';
...
function createContainerRoutes(useCase: ContainerUseCase): Hono {
  ...
  containers.post('/', async (c) => {
    try {
      ...
      const result = await useCase.createContainer(validated);
      return c.json(result, 201);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return c.json({ error: 'Validation error', message: error.errors }, 400);
      }
      return c.json({ error: 'Internal error', message: 'Failed to create container' }, 500);
    }
  });
